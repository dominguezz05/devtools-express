...
const GitIcon = ({ className = "w-6 h-6 text-blue-600" }) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
    ...
    <path strokeLinecap="round" strokeLinejoin="round"
      d="M15 9l-3 3m0 0l-3 3m3-3l3 3m-3-3l-3-3m9.75-3h-15a.75.75 0 00-.75.75v12.75c0 .414.336.75.75.75h15a.75.75 0 00.75-.75V6.75a.75.75 0 00-.75-.75z"
    />
  </svg>
);
...
// Componente pequeño y reutilizable para botones de copiar en línea
const InlineCopyButton = ({ textToCopy }) => {
  ...
  const handleCopy = () => {
    copyToClipboard(textToCopy,
      () => setStatus('copied'),
      () => setStatus('error') // Podrías tener un estado 'error' si lo necesitas
    );
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      className={`${secondaryButtonClasses(status === 'copied').replace('text-xs','text-[11px] leading-4').replace('px-3 py-1.5','px-2 py-1')} min-w-[70px]`} // Clases más pequeñas
...
function GitCommandHelper() {
  ...
  const [commands, setCommands] = useState(initialCommands); // Para futura edición/adición
  ...
  return (
    <div className={cardContainerClasses}>
      ...
      <p className={toolDescriptionClasses}>
        Copia rápidamente comandos comunes de Git o prueba los tuyos.
      </p>
      ...
      <div className="mt-8 pt-6 border-t border-gray-200">
        ...
        >
          {customCopyStatus === 'copied' ? <CheckIconMini/> : <CopyIconMini/> }
          <span className="ml-2">
            {customCopyStatus === 'copied' ? '¡Comando Copiado!' : customCopyStatus === 'error' ? 'Error al Copiar' : 'Copiar Personalizado'}
          </span>
        </button>
      </div>
    </div>

...
function HistoryPanel({ history, onSelect, onClear, maxSnippetLength = 80 }) {
  ...
  return (
    <div className="mt-6 pt-6 border-t border-gray-200">
      <div className="flex justify-between items-center mb-2">
        <h3 className="text-md font-medium text-gray-700">Historial Reciente</h3>
        <button
          type="button"
          onClick={onClear}
      ...
      <div className="space-y-1 max-h-36 overflow-y-auto pr-2" style={{ scrollbarWidth: 'thin' }}>
        {history.map((item, index) => (
          <button
            ...
            title="Usar esta entrada"
          >
            <code className="text-xs text-gray-600 break-all whitespace-pre-wrap">
              {item.length > maxSnippetLength ? `${item.substring(0, maxSnippetLength)}...` : item}
            </code>
          </button>
        ))}
      </div>
